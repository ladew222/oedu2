...
module.exports = function (grunt) {
  ...
  grunt.registerTask('compile', 'Compiles the Drupal/Bootstrap override CSS files for the base theme.', function () {
    ...
    grunt.registerTask('compile:overrides', function () {
      ...
      var SassPluginAutoPrefix = require('sass-plugin-autoprefix');
      var SassPluginCleanCSS = require('sass-plugin-clean-css');
      var sassPlugins = [
        new SassPluginCleanCSS({
          advanced: true
        }),
        new SassPluginAutoPrefix({
          browsers: [
            "Android 2.3",
            "Android >= 4",
            "Chrome >= 20",
          ...
          for (var i = 0; i < libraries[library][version].length; i++) {
            ...
            (function (library, versions, version, theme, total) {
              queue.push(function (done) {
                var sassPaths = [path.join(librariesPath)];
                var latestVersion = [].concat(versions).pop();
                var latestVariables = path.join(latestVersion, 'bootstrap', 'sass', 'variables.less');
                var latestMixins = path.join(latestVersion, 'bootstrap', 'sass', 'mixins.sass');
                var themeVariables = path.join(version, library, (library === 'bootstrap' ? 'sass' : theme), 'variables.scss');
                var backupVariables = path.join(version, 'bootstrap', 'sass', 'variables.scss');
                ...
                var imports = [
                  ...
                  '@import "' + path.join('starterkits', 'sass', 'less', 'overrides.less') + '"'
                ];
